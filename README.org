...
This project exposes myCBR through a Spring Boot REST API with
...
#+BEGIN_EXAMPLE
    java -DMYCBR.PROJECT.FILE={path to the project file} -jar ./target/mycbr-rest-2.1.jar
#+END_EXAMPLE
...
- /casebases provides the name(s) of the case bases associated with the
  project
- /retrieval provides the similarity-based retrieval either by specifying
  symbols or an id of existing cases
- /attributes provides a list of attributes and their value types
- /concepts/{conceptID}/attributes/{attributeID} provides attribute metadata
  and value ranges ## POST Requests
...
- The rest of the API is independent from the project, hence only the
  parameters such as case base and concept names have to be adapted to
  the new project
...
- POST: POST operations should be used when the REST api user does not know exactly
  ...
  behaviour or bad documentation. This operation should return a URL that
